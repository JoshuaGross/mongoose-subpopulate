...
var wrapSchema = exports.wrapSchema = function wrapSchema (schema) {
  ...
  if (schema.findById && !schema.findById.spandexWrapped) {
    ...
    schema.findById.spandexWrapped = true;
  }
  if (schema.find && !schema.find.spandexWrapped) {
    ...
    schema.find.spandexWrapped = true;
  }
  if (schema.findOne && !schema.findOne.spandexWrapped) {
    ...
    schema.findOne.spandexWrapped = true;
  }

  if (schema.save && !schema.save.spandexWrapped) {
    ...
    schema.save.spandexWrapped = true;
  }

  // Chaining support
  if (schema.populate && !schema.populate.spandexWrapped) {
    ...
    schema.populate.spandexWrapped = true;
  }
  if (schema.exec && !schema.exec.spandexWrapped) {
    ...
    schema.exec.spandexWrapped = true;
  }

  schema.spandexWrapped = true;
  ...
  if (!schema.spandexWrappedConstructor) {
    ...
    newSchema.spandexWrappedConstructor = true;
    return newSchema;
  }
    ...
    if (typeof object[i] === 'function') {
      if ((!i.match(/^to/) || i.match(/^toObject/)) && 'inspect' != i) {
        (function (attr) {
          wrappedObject[attr] = function () {
            object[attr].apply(object, arguments);
          }
        })(i);
      }
    } else {
      (function (attr) {
        wrappedObject.__defineGetter__(attr, function () {
          ...
          if (!/(\/mongoose\/|\/mongoose-subpopulate\/)/.test(getCallerDetails().file)) {
